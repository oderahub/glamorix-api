  ...
  try {
    ...
    if (variants.length > 0) {
      const variantData = variants.map((variant, index) => ({
        ...
        material: variant.material || 'N/A',
        price: variant.price || price,
        // Use variant's stockQuantity if provided, otherwise use product's stockQuantity
        stockQuantity:
          variant.stockQuantity !== undefined ? variant.stockQuantity : stockQuantity || 0,
        sku: `${slug}-${index + 1}`,
        createdAt: new Date(),
        updatedAt: new Date(),
      }));
  ...
  try {
    ...
    if (req.body.variants) {
      ...
      const variantData = req.body.variants.map((variant, index) => ({
        ...
        material: variant.material || 'N/A',
        price: variant.price || product.price,
        // Use variant's stockQuantity if provided, otherwise use updated product stockQuantity (if provided), or fallback to 0
        stockQuantity:
