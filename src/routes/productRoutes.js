...
import { createProduct, updateProduct, deleteProduct, updateStock, archiveProduct, restoreProduct } from '../controllers/productController.js';
...
router.post('/admin/products', authenticateToken, requireRole(ROLES.ADMIN), validateRequest(productSchema), createProduct);
router.patch('/admin/products/:id', authenticateToken, requireRole(ROLES.ADMIN), validateRequest(updateProductSchema), updateProduct);
router.delete('/admin/products/:id', authenticateToken, requireRole(ROLES.ADMIN), deleteProduct);
router.patch('/admin/products/:id/stock', authenticateToken, requireRole(ROLES.ADMIN), validateRequest(Joi.object({ stockQuantity: Joi.number().integer().min(0).required() })), updateStock);
router.patch('/admin/products/:id/archive', authenticateToken, requireRole(ROLES.ADMIN), archiveProduct);
